...
# substitute your application name for instances of comptext

# Variables
!define comptext_DESCRIPTION "${comptext_NAME} ${comptext_VERSION}"
!define comptext_STRING "${comptext_NAME}-${comptext_VERSION}"

!define PRODUCT_BINARY "${comptext_BINARY}"
!define PRODUCT_NAME "${comptext_NAME}"
!define PRODUCT_VERSION "${comptext_VERSION}"
!define PRODUCT_STRING "${comptext_STRING}"
!define PRODUCT_DESCRIPTION "${comptext_DESCRIPTION}"
...
VIAddVersionKey FileDescription "${comptext_DESCRIPTION} installer"
...
Section -install
    ...
    WriteRegStr HKLM "${REG_UNINSTALL_PATH}" "Publisher" "comptext developers"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "${SM_PATH}\${comptext_NAME}.lnk" "$INSTDIR\${comptext_BINARY}" "" "$INSTDIR\${comptext_BINARY}" 0
#	CreateShortCut "${SM_PATH}\${comptext_NAME} Beginners' Guide.lnk" "${GUIDE_URL}"
#	CreateShortCut "${SM_PATH}\${comptext_NAME} Documentation.lnk" "${comptext_DOCS_URL}"
    CreateShortCut "${SM_PATH}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
SectionEnd

Section "Desktop Shortcuts"
	CreateShortCut "$DESKTOP\${comptext_DESCRIPTION}.lnk" "$INSTDIR\${comptext_BINARY}" "" \
		"$INSTDIR\${comptext_BINARY}" 0
SectionEnd

# This is unselected by default
Section /o "Quick Launch Shortcuts"
	CreateShortCut "$QUICKLAUNCH\${comptext_DESCRIPTION}}.lnk" "$INSTDIR\${comptext_BINARY}" "" \
		"$INSTDIR\${comptext_BINARY}" 0
SectionEnd
...
# Remove files and uninstaller
	Delete /REBOOTOK $INSTDIR\${comptext_BINARY}
    Delete /REBOOTOK $INSTDIR\uninstall.exe

# Remove shortcuts, if any
    Delete "${SM_PATH}\*.*"
	Delete "$DESKTOP\${comptext_DESCRIPTION}.lnk"
	Delete "$QUICKLAUNCH\${comptext_DESCRIPTION}.lnk"
